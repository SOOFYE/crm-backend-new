...
export class Campaigntables1725055485654 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        // Ensure enum type is created only if it doesn't already exist
        await queryRunner.query(`
            DO $$
            BEGIN
                IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'campaign_data_status_enum') THEN
                    CREATE TYPE "public"."campaign_data_status_enum" AS ENUM('pending', 'failed', 'success');
                END IF;
            END
            $$;
        `);
        ...
        await queryRunner.query(`
            CREATE TABLE "campaign_data" (
                ...
                "status" "public"."campaign_data_status_enum" NOT NULL DEFAULT 'pending',
                "data" jsonb,
                "createdAt" TIMESTAMP NOT NULL DEFAULT now(),
                "updatedAt" TIMESTAMP NOT NULL DEFAULT now(),
                "deletedAt" TIMESTAMP,
                "campaignTypeId" uuid,
                CONSTRAINT "PK_202a28f9808768e8f1d9b8ac03e" PRIMARY KEY ("id")
            );
        `);

        // Create "original_campaign_data" table
        await queryRunner.query(`
            CREATE TABLE "original_campaign_data" (
                "id" uuid NOT NULL DEFAULT uuid_generate_v4(),
                "name" character varying NOT NULL,
                "s3Url" character varying(255) NOT NULL,
                "duplicateFieldCheck" text NOT NULL,
                "createdAt" TIMESTAMP NOT NULL DEFAULT now(),
                "updatedAt" TIMESTAMP NOT NULL DEFAULT now(),
                "deletedAt" TIMESTAMP,
                "campaignTypeId" uuid,
                "preprocessedDataId" uuid,
                CONSTRAINT "PK_78ce4e6faf0e27d995fa10b8f92" PRIMARY KEY ("id")
            );
        `);
        ...
        await queryRunner.query(`
            CREATE TABLE "campaign_types" (
                ...
                "updatedAt" TIMESTAMP NOT NULL DEFAULT now(),
                "deletedAt" TIMESTAMP,
                CONSTRAINT "PK_681677e8838f686df37b6046dc5" PRIMARY KEY ("id")
            );
        `);

        // Create "campaigns_agents_users" table
        await queryRunner.query(`
            CREATE TABLE "campaigns_agents_users" (
                "campaignsId" uuid NOT NULL,
                "usersId" uuid NOT NULL,
                CONSTRAINT "PK_244d39042c85c4297871e16c4d6" PRIMARY KEY ("campaignsId", "usersId")
            );
        `);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        // Drop enum type
        await queryRunner.query(`DROP TYPE IF EXISTS "public"."campaign_data_status_enum";`);
    }
}